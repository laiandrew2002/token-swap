...
describe('TokenAmountInput', () => {
  ...
  it('displays token amount value', () => {
    const { container } = render(<TokenAmountInput {...defaultProps} tokenAmount="100" />)
    const input = container.querySelector('input[placeholder="0.00"]') as HTMLInputElement
    expect(input?.value).toBe('100')
  })

  it('calls onTokenAmountChange when token amount is typed', async () => {
    const handleChange = jest.fn()
    const { container } = render(<TokenAmountInput {...defaultProps} onTokenAmountChange={handleChange} token={mockToken} />)
  ...
  it('displays USD amount', () => {
    const { container } = render(<TokenAmountInput {...defaultProps} usdAmount="1000" />)
  ...
  it('calls onUsdAmountChange when USD amount is typed', async () => {
    const handleChange = jest.fn()
    const { container } = render(<TokenAmountInput {...defaultProps} onUsdAmountChange={handleChange} token={mockToken} />)
  ...
  it('shows retry button for retryable errors', () => {
    ...
    render(<TokenAmountInput {...defaultProps} error="Network error" onRetry={onRetry} />)
    expect(screen.getByText('Try Again')).toBeInTheDocument()
  })

  it('does not show retry button for non-retryable errors', () => {
    const onRetry = jest.fn()
    render(<TokenAmountInput {...defaultProps} error="API key not configured" onRetry={onRetry} />)
  ...
  it('disables inputs when token is not selected', () => {
    const { container } = render(<TokenAmountInput {...defaultProps} />)
    const tokenInput = container.querySelector('input[placeholder="0.00"]') as HTMLInputElement
    expect(tokenInput?.disabled).toBe(true)
  })

  it('enables inputs when token is selected', () => {
    const { container } = render(<TokenAmountInput {...defaultProps} token={mockToken} />)
    const tokenInput = container.querySelector('input[placeholder="0.00"]') as HTMLInputElement
    expect(tokenInput?.disabled).toBe(false)
  })

  it('formats large USD amounts with abbreviations when not focused', () => {
    const { container } = render(<TokenAmountInput {...defaultProps} usdAmount="1500000" />)
  ...
  it('shows loading skeleton when isLoading is true', () => {
    const { container } = render(<TokenAmountInput {...defaultProps} isLoading={true} />)
