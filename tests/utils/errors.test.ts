...
describe('getUserFriendlyError', () => {
  it('returns user-friendly message for API key errors', () => {
    ...
    expect(result).toBe('API configuration is missing. Please check your settings.')
  })

  it('returns user-friendly message for network errors', () => {
    const error = 'Failed to fetch: Network error'
    const result = getUserFriendlyError(error)
    expect(result).toBe('Unable to connect to the service. Please check your internet connection and try again.')
  })

  it('returns user-friendly message for token not found', () => {
    const error = 'Token USDC not found on chain 1'
    const result = getUserFriendlyError(error)
    expect(result).toBe('Token information could not be found. Please try selecting a different token.')
  })

  it('returns user-friendly message for price not found', () => {
    const error = 'Price not found for token 0x123 on chain 1'
    const result = getUserFriendlyError(error)
    expect(result).toBe('Price data is currently unavailable. Please try again in a moment.')
  })
